import socket,argparse,sys,os,pathlib,random,threading,itertools,base64,platform,json,tcp_date
...
class TCP:
    ...
    def tcp_attack(self,port_path,send_int,send_path):
            ...
            while True:
                ...
                sys.stdout.flush()
                self.tcp_attack_1(port_path,send_int)
                self.tcp_attack_2(port_path)
                self.tcp_attack_3(port_path,send_path)
        ...
        try:
            ...
            if platform.system() == "Windows":
                for attack in range(int(parse.task)):
                    tcp.tcp_attack(port_path,int(parse.send),send_path)
            else:
                for attack in range(int(parse.task)):

                    th = threading.Thread(target=tcp.tcp_attack,args=(port_path,int(parse.send),send_path,))
                    th.start()
                    while True:
                        th.join(1)
